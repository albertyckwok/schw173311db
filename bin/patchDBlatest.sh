#!/bin/bash
if [ "$#" -lt 1 ]; then
        echo $0 dbHomeId [region] [lastest-n]
	echo The following will patch the home to n-1 of PSU default is IAD and latest, i.e. n-0
        echo $0 ocid1.dbhome...... us-ashburn-1 1
        exit
fi
dbHomeId=$1
if [ "$2" == "" ]; then
...
patchId=`oci db patch list-db-home --all --region $TF_VAR_region --db-home-id $dbHomeId| jq -r '.data[].id'|head -$nMinus|tail -1`
if [ "$patchId" == "" ]; then
